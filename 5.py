...
# ==========================================
# 1. 构造一个“真实的”地质数据集（加入更多因子）
# ==========================================
n_samples = 2000

data = {
    'Slope': np.random.normal(30, 10, n_samples),                  # 坡度
    'Rainfall': np.random.normal(100, 30, n_samples),              # 降雨量
    'Lithology': np.random.choice(['Granite', 'Mudstone', 'Limestone'], n_samples),
    'Fault_Distance': np.random.uniform(0, 5000, n_samples),       # 距断层距离

    # 新增因子
    'TWI': np.random.normal(8, 2, n_samples),                      # 地形湿度指数
    'River_Distance': np.random.uniform(0, 3000, n_samples),       # 距水系距离
    'Road_Distance': np.random.uniform(0, 2000, n_samples),        # 距道路距离
    'Curvature': np.random.normal(0, 0.5, n_samples)               # 地形曲率(凹负凸正)
}
...
def simulate_landslide(row):
    ...
    # 坡度越大越危险
    score += (row['Slope'] - 20) * 0.5

    # 降雨越大越危险
    score += (row['Rainfall'] - 80) * 0.2
    ...
    # 距水系：越近越危险
    if row['River_Distance'] < 300:
        score += 10
    elif row['River_Distance'] < 800:
        score += 5

    # 距道路：越近可能切坡、排水破坏 → 稍微危险
    if row['Road_Distance'] < 200:
        score += 8
    elif row['Road_Distance'] < 500:
        score += 4

    # 地形曲率：凹地(负)更危险，凸地(正)略安全
    score += -5 * row['Curvature']   # Curvature<0 → score 增大；>0 → 减少
...
print(f"\n基础 XGBoost 模型 准确率: {acc_base*100:.2f}%")
print("\n分类报告：")
...
plt.plot(fpr_base, tpr_base, lw=2, label=f'Base ROC (AUC = {roc_auc_base:.2f})')
plt.plot([0, 1], [0, 1], lw=2, linestyle='--', label='Random Guess')
plt.xlim([0.0, 1.0])
plt.ylim([0.0, 1.05])
plt.xlabel('False Positive Rate (误报率)')
plt.ylabel('True Positive Rate (命中率)')
plt.title('Receiver Operating Characteristic (ROC) - 基础 XGBoost 滑坡预测模型')
plt.legend(loc="lower right")
plt.grid(alpha=0.3)
plt.show()

# ==========================================
# 6. GridSearchCV 自动调参（以 AUC 为目标）—— XGBoost
# ==========================================
param_grid = {
...
print(f"\n最优 XGBoost 模型在测试集上的准确率: {acc_best*100:.2f}%")
...
plt.xlabel('False Positive Rate (误报率)')
plt.ylabel('True Positive Rate (命中率)')
plt.title('Receiver Operating Characteristic (ROC) - 最优 XGBoost 滑坡预测模型')
plt.legend(loc="lower right")
plt.grid(alpha=0.3)
plt.show()
...
grid_data = pd.DataFrame({
    ...
    # TWI：假设与 y 坐标有关，越北越湿
    'TWI': 8 + 0.02 * flat_y + np.random.normal(0, 0.5, N*N),

    # 假设左侧 x=10 有一条河
    'River_Distance': np.abs(flat_x - 10) * 50,

    # 假设下方 y=20 有一条公路
    'Road_Distance': np.abs(flat_y - 20) * 50,

    # 曲率：纯随机，实际可用 DEM 计算
    'Curvature': np.random.normal(0, 0.5, N*N),
